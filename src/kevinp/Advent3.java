...
public class Advent3 {
  ...
  static int crossingDistance(String one, String two) {
    var first = one.split(",");
    var second = two.split(",");
    var visited = new HashMap<Location, Integer>();
    var initialRun = true;
    var minDistance = Integer.MAX_VALUE;
    var bestLocation = new Location(0, 0);
    for (var runs : List.of(first, second)) {
      var location = new Location(0, 0);
      var steps = 0;
      for (var movement : runs) {
        System.out.println("at " + location);
        var dir = movement.substring(0, 1);
        var dist = Integer.parseInt(movement.substring(1));
        for (int i = 0; i < dist; i++) {
          ...
          if (initialRun) {
            if (!visited.containsKey(location))
              visited.put(location, steps);
          } else {
            if (visited.containsKey(location)) {
              int combinedSteps = visited.get(location) + steps;
              if (location.distance() > 0 && combinedSteps < minDistance) {
                minDistance = combinedSteps;
